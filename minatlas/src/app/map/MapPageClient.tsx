...
export default function MapPageClient() {
  ...
  const deepLinkZoomedSiteIdRef = useRef<string | null>(null);
  ...
  const deepLinkedSiteId = searchParams.get("site")?.trim() ?? "";
  ...
  const filteredSites = useMemo(
    ...
    [mineSites, normalizedSearchQuery, selectedCommodities, selectedStates, selectedStatuses],
  );

  const visibleSites = useMemo(
    () => selectMineSitesForMap(filteredSites, maxPointsRendered, mapTelemetry.zoomLevel),
    [filteredSites, mapTelemetry.zoomLevel, maxPointsRendered],
  );

  const mapSites = useMemo(() => {
    if (!layersEnabled) return [];
    if (!selectedSite) return visibleSites;
    if (visibleSites.some((site) => site.id === selectedSite.id)) return visibleSites;
    return [...visibleSites, selectedSite];
  }, [layersEnabled, selectedSite, visibleSites]);

  const deepLinkedSite = useMemo(() => {
    if (!deepLinkedSiteId) return null;
    return mineSites.find((site) => site.id === deepLinkedSiteId) ?? null;
  }, [deepLinkedSiteId, mineSites]);

  useEffect(() => {
    if (!deepLinkedSite) return;
    setLayersEnabled(true);
    setSelectedSite((current) => (current?.id === deepLinkedSite.id ? current : deepLinkedSite));
  }, [deepLinkedSite]);

  useEffect(() => {
    if (!deepLinkedSiteId) {
      deepLinkZoomedSiteIdRef.current = null;
    }
  }, [deepLinkedSiteId]);

  useEffect(() => {
    if (!mapControls || !selectedSite || !deepLinkedSiteId) return;
    if (selectedSite.id !== deepLinkedSiteId) return;
    if (deepLinkZoomedSiteIdRef.current === selectedSite.id) return;
    mapControls.zoomToSiteWithFocus(selectedSite);
    deepLinkZoomedSiteIdRef.current = selectedSite.id;
  }, [deepLinkedSiteId, mapControls, selectedSite]);

  useEffect(() => {
    if (!selectedSite) return;
    const stillMatchesActiveFilters = filteredSites.some((site) => site.id === selectedSite.id);
    if (!stillMatchesActiveFilters) {
      setSelectedSite(null);
    }
  }, [filteredSites, selectedSite]);

  useEffect(() => {
    if (!selectedSite || deepLinkedSiteId) return;
    deepLinkZoomedSiteIdRef.current = null;
  }, [deepLinkedSiteId, selectedSite]);
