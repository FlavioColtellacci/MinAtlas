...
export const MINE_SITE_PAGE_SIZE = 1000;
export const MINE_SITE_PAGE_COUNT = 4;
...
export function normalizeCommodity(value: string) {
...
export function getImportanceScore(site: MineSiteRow, commodity: string[]) {
  return Math.round(
...
export function mineSiteRowToMineSite(site: MineSiteRow): MineSite {
...
export async function fetchAllMineSites(supabase: SupabaseClient<AppDatabase, "api">): Promise<MineSite[]> {
  ...
  return pages.flatMap((page) => (page.data ?? []) as MineSiteRow[]).map(mineSiteRowToMineSite);
}
